...
public class GameEvent : MonoBehaviour
{
			...
			if (GameEvent.Instance.gameStatus != value) {
				if (value == GameState.WinProccess) {
					...
					if (PlayerPrefs.GetInt("Won") == 1) {
						...
						if (DatabaseManager.Instance.GetLocalData().data.OpenLevel + 1 <= LevelsMap._instance.MapLevels.Count) {
							Debug.Log("");
	...
	IEnumerator WinAction()
	{
		...
		if (DatabaseManager.Instance.GetLocalData().data.starsCount[string.Format("Level.{0:000}.StarsCount", mainscript.Instance.currentLevel)] < mainscript.Instance.stars)
			DatabaseManager.Instance.GetLocalData().data.starsCount[string.Format("Level.{0:000}.StarsCount", mainscript.Instance.currentLevel)] = mainscript.Instance.stars;



		if(DatabaseManager.Instance.GetLocalData().data.scores.ContainsKey("Score" + mainscript.Instance.currentLevel)){
			if (DatabaseManager.Instance.GetLocalData().data.scores["Score" + mainscript.Instance.currentLevel] < mainscript.Score) {
				DatabaseManager.Instance.GetLocalData().data.scores["Score" + mainscript.Instance.currentLevel] = mainscript.Score;

			}
		}
		else{
			LocalData data = DatabaseManager.Instance.GetLocalData();
			data.data.scores.Add("Score" + mainscript.Instance.currentLevel,mainscript.Score);
			DatabaseManager.Instance.UpdateData(data);
		}

		claimTokenBTN.SetActive(mainscript.Instance.stars == 3);
