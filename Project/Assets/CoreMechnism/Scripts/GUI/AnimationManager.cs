...
public class AnimationManager : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        {
            //		for (int i = 1; i <= 3; i++) {
            //			//transform.Find ("Image").transform.Find ("Stars").Find ("Star" + i).gameObject.SetActive (false);
            //		}
            //		int stars = PlayerPrefs.GetInt (string.Format ("Level.{0:000}.StarsCount", PlayerPrefs.GetInt ("OpenLevel")), 0);
            //	if (stars > 0) {
            //			for (int i = 1; i <= stars; i++) {
            //	
            //	transform.Find ("Image").transform.Find ("Stars/Star" + i).gameObject.SetActive (true);
            //			}
            //
            //		} else {
            //			for (int i = 1; i <= 3; i++) {
            //transform.Find ("Image").transform.Find ("Stars/Star" + i).gameObject.SetActive (false);
            //			}
    ...
    {
#if UNITY_IOS
		Application.OpenURL("https://itunes.apple.com/app/id1320437054");
#else
        Application.OpenURL("https://play.google.com/store/apps/details?id=com.tgs.mpbs");
#endif
    }
    ...
    public void GoToMap(int type = 0)
    {
        SoundBase.Instance.GetComponent<AudioSource>().PlayOneShot(SoundBase.Instance.click);
        // if (MenuManager.Instance != null)
        //  (MenuManager.Instance.Loading);
        if (type == 1) SceneManager.LoadScene("Splash2");
        else SceneManager.LoadScene("Splash4");
    }
    ...
    public void HandleRewardedComplete()
    {
        if (gameObject.name == "PreFailedBanner")
        {
    ...
    public void ExchangeCoins(int tokenValue){ 
        if(int.TryParse(CoreChainManager.userTokenBalance,out int _currentBalance))
        {
            if(_currentBalance>= tokenValue){
                CoreChainManager.Instance.ExchangeToken(tokenValue);
            }
            else{
                MessageBox.Instance.showMsg("Not Enough Tokens!");
            }
        }        
    }
    public void Buy(GameObject pack)
    {
        ...
        if (pack.name == "Pack1")
        {
            ...
            CoreChainManager.Instance.CoinBuyOnSendContract(0);
...
/*#if UNITY_INAPPS
        ...
        if (pack.name == "Pack2")
        {
            ...
            CoreChainManager.Instance.CoinBuyOnSendContract(1);
...
/*#if UNITY_INAPPS
        ...
        if (pack.name == "Pack3")
        {
            ...
            CoreChainManager.Instance.CoinBuyOnSendContract(2);
			//CloseMenu();
			return;
...
/*#if UNITY_INAPPS
        ...
        if (pack.name == "Pack4")
        {
            ...
            CoreChainManager.Instance.CoinBuyOnSendContract(3);
			//CloseMenu();
			return;
...
/*#if UNITY_INAPPS
    ...
    public void Quit()
    {
        ...
        else if (!MenuManager.Instance.ManuSpinner.activeSelf)
        {
    ...
    public void exitScreen(int type = 0)
    {
        if (type == 0) Application.Quit();
        if (type == 1)
        {
            MenuManager.Instance.ExitMenu.SetActive(false);
    ...
    {
        if (MenuManager.Instance.SpinnerMsg.activeSelf)
        {
            MenuManager.Instance.SpinnerMsg.SetActive(false);
        }
        else if (MenuManager.Instance.ManuSpinner.activeSelf)
        {
            MenuManager.Instance.ManuSpinner.SetActive(false);
        }
        else
        {
            MenuManager.Instance.ManuSpinner.SetActive(true);
        }
    }
